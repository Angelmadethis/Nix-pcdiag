...
/// <summary>
/// Implements the <c>pcdiag check packet-loss</c> command: runs the packet-loss and
/// latency check and prints its detailed report. Optional positional arguments
/// override the internet test target(s). No interaction required.
/// </summary>
public static class PacketLossCommand
{
    public static async Task<int> RunAsync(IReadOnlyList<string> args, CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            var check = new PacketLossCheck(targetOverrides: targets);
        ...
        {
            Console.Error.WriteLine($"pcdiag check packet-loss failed: {ex.Message}");
            return 1;
        }
    }
}